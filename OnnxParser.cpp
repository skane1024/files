    ...
    namespace
    {
                                ...
                                const armnn::CheckLocation &location)
        {
            bool isValid = std::any_of(validInputTypes.begin(),
                                       validInputTypes.end(),
                                       [&actualValue](onnx::TensorProto::DataType x)
...
#define VALID_INPUTS(NODE, VALID_INPUTS)         \
                                                              ...
                                                              const onnx::TensorProto::DataType &dataType)
    {
        ...
        bool needCompute = std::any_of(outNames.begin(),
                                       ...
                                       {
                                           return (m_TensorsInfo.count(name) == 0 || m_TensorsInfo[name].m_info == nullptr || m_TensorsInfo[name].m_info->GetShape().GetDimensionality() == Dimensionality::NotSpecified);
                                       });
                                      ...
                                      armnn::Optional<armnn::PermutationVector &> permutationVector)
    {
        ...
        // float scale = 1.0f;
        // int zeropoint = 0;
        if (m_jsonRoot.isMember(name))
        {
            Json::Value prop = m_jsonRoot[name];
            std::string type = prop["type"].asString();
            tensorInfo.SetDataType(mapDataType(type));

            if (prop.isMember("perchannel"))
            {
                std::vector<float> scales;
                const Json::Value &scales_data = prop["scales"];
                for (Json::ArrayIndex i = 0; i < scales_data.size(); i++)
                {
                    scales.push_back(scales_data[i].asFloat());
                    std::cout << scales_data[i].asFloat() << " ";
                }

                int zeropoint = prop["zeropoint"].asInt();
                tensorInfo.SetQuantizationDim(static_cast<unsigned int>(scales.size()));
                tensorInfo.SetQuantizationScales(scales);
                tensorInfo.SetQuantizationOffset(zeropoint);
                std::cout << "scales.size()" << scales.size() << std::endl;
            }
            else
            {
                float scale = 0.001f; // prop["scale"].asFloat();
                int zeropoint = prop["zeropoint"].asInt();
                tensorInfo.SetQuantizationScale(scale);
                tensorInfo.SetQuantizationOffset(zeropoint);
            }
        }

        size_t pos = name.find("reshape_output_");
        if (pos != std::string::npos)
        {
            std::string origin_name = name.substr(15);
            if (m_jsonRoot.isMember(origin_name))
            {
                Json::Value prop = m_jsonRoot[origin_name];
                std::string type = prop["type"].asString();
        ...
        // 13361  完成数据的量化
        // auto srcFloatDataPtr = onnxTensor.float_data().data();
        auto srcFloatDataPtr1 = onnxTensor.mutable_float_data();
        google::protobuf::RepeatedField<char> *pdataPtr = reinterpret_cast<google::protobuf::RepeatedField<char> *>(srcFloatDataPtr1);
        for (int i = 0; i < onnxTensor.float_data_size(); i++)
        {
            (*pdataPtr)[i] = 55; // static_cast<int8_t>(std::round((*srcFloatDataPtr1)[i] / scale) + float(zeropoint));
        }
        // 13361
    ...
    {
        std::cout << "OnnxParserImpl::CreateNetworkFromBinaryFile" << std::endl;
        // 13361
        ParseJsonParameters();
        // 13361
    ...
    {
        std::cout << "OnnxParserImpl::CreateNetworkFromModel(onnx::ModelProto& model)" << std::endl;
    ...
    void OnnxParserImpl::DetectFullyConnected()
    {
                                     ...
                                     int &nodeIndex)
        {
            ...
            if (matmulIt != m_OutputsMap.end() && matmulIt->second.first->op_type() == "MatMul" && m_TensorsInfo[constInput].isConstant())
            {
        ...
        for (int nodeIndex = 0; nodeIndex < m_Graph->node_size(); nodeIndex++)
        {
            ...
            if (node->op_type() == "Add")
            {
                    ...
                    matmulAndConstant(node->input(1), node->input(0), matmulIndex))
                {
                    ...
                    m_OutputsFusedAndUsed[static_cast<size_t>(matmulIndex)].fusedWithNodes.push_back(static_cast<size_t>(nodeIndex));

                    m_OutputsFusedAndUsed[static_cast<size_t>(nodeIndex)].fusedWithNodes.push_back(static_cast<size_t>(matmulIndex));
                }
            }
        }

        for (auto output : m_Graph->output())
        { // Add usages as output of the graph in count of usages
    ...
    void OnnxParserImpl::CreateConstantLayer(const std::string &tensorName, const std::string &layerName)
    {
        ...
        // 13361
        // addd output
        auto tensor_info = armnnTensor.first.GetInfo();

        if (m_jsonRoot.isMember(tensorName))
        {
            Json::Value prop = m_jsonRoot[tensorName];
            std::string type = prop["type"].asString();
            float scale = prop["scale"].asFloat();
            int zeropoint = prop["zeropoint"].asInt();
            tensor_info.SetDataType(mapDataType(type));
            tensor_info.SetQuantizationScale(scale);
            tensor_info.SetQuantizationOffset(zeropoint);
        }

        // 13361

        layer->GetOutputSlot(0).SetTensorInfo(tensor_info);
    ...
    {
        std::cout << "OnnxParserImpl::ParseConv" << std::endl;
        ...
        // 13361
        if (m_jsonRoot.isMember(node.output(0))) // node.output(0)   tensor的名字
        {
            Json::Value prop = m_jsonRoot[node.output(0)];
            std::string type = prop["type"].asString();
            float scale = prop["scale"].asFloat();
    ...
    {
        std::cout << "OnnxParserImpl::SetupInputLayers" << std::endl;
        ...
        for (int inputIndex = 0; inputIndex < m_Graph->input_size(); ++inputIndex)
        {
            ...
            if (!m_TensorsInfo[input.name()].isConstant())
            {
                ...
                // 13361
                // 修改 input   layer  输入的量化参数
                if (m_jsonRoot.isMember(input.name()))
                {
                    Json::Value prop = m_jsonRoot[input.name()];
                    std::string type = prop["type"].asString();
                    float scale = prop["scale"].asFloat();
                    int zeropoint = prop["zeropoint"].asInt();
                    tensorInfo.SetDataType(mapDataType(type));
    ...
    {
        std::cout << "OnnxParserImpl::SetupOutputLayers" << std::endl;
    ...
    // 13316
    // 读取解析json 量化参数
    void OnnxParserImpl::ParseJsonParameters()
    {
        std::ifstream json_file("config.json");
        json_file >> m_jsonRoot;
        // 解析JSON对象中的每个属性
        for (auto &name : m_jsonRoot.getMemberNames())
        {
            Json::Value prop = m_jsonRoot[name];
            std::string type = prop["type"].asString();
            float scale = prop["scale"].asFloat();
            ...
            // 输出解析结果
            std::cout << name << ": type=" << type << ", scale=" << scale << ", zeropoint=" << zeropoint << std::endl;
        }
    }
    // 13361
